...
framework and read-only Git-state reporting; no other worker participates in
that closure update.
...
- **Heavy**: deployment-state work orchestrated through specialized workers.
  Read `<Codex home>/codex_workflow/heavy_route.md`.
...
- For every substantive Medium or Heavy deployment, read the selected route and
  `<Codex home>/codex_workflow/explorer_companion.md`, then initialize or reuse
  the single persistent Explorer.
...
- Before the final response that completes, pauses, or blocks each substantive
  ...
  `<Codex home>/codex_workflow/end_of_session.md` exactly once. Its worker
  inherits recent main-agent context and performs the complete
  documentation-framework update. The handoff is not a user command.

## Platform Paths

Resolve the Codex home from a non-empty `CODEX_HOME` environment variable when
it is set; otherwise use `~/.codex`. Installed workflow resources live under
`<Codex home>/codex_workflow`; never assume `~/.codex` when `CODEX_HOME` is
set. Workflow documents use `/` as a platform-neutral separator. Translate
paths to the current operating system and shell when running filesystem
commands.
